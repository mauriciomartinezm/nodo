...
export const finalizarTrabajo = async (req, res) => {
  console.log("Peticion recibida en /finalizarTrabajo Cuerpo de la petición: ");
  ...
  try {
    ...
    if (id_publicacion) {
      const trabajador = await db.query('SELECT id_trabajador FROM postulacion WHERE id_publicacion = $1 AND estado = $2', [id_publicacion, 'aceptado']);
      var id_trabajador = trabajador.rows[0]['id_trabajador'];
      ...
      id_postulacion = postulacion.rows[0]['id']
      console.log("id postulacion: ");
      console.log(id_postulacion);
      mensajes.push("Publicación finalizada");
      await db.query(
      ...
      mensajes.push("Postulación finalizada");
      return res.status(200).json({ message: mensajes.join(" y ") + "." });
    }
    ...
    if (id_postulacion) {
      ...
      //console.log(publicacion);
      const idPublicacion = postulacion.rows[0]['id_publicacion'];
      const publicacion = await db.query(

        'SELECT * from publicacion WHERE id = $1',
        [idPublicacion]
      );
      const clienteId = publicacion.rows[0]['id_cliente'];
      const titulo = publicacion.rows[0]['titulo'];
      ...
      await guardarNotificacion(
        ...
        `El trabajador ha marcado como terminado el trabajo "${titulo}". Por favor, revisa los resultados, entra a la publicación y confirma si estás de acuerdo para finalizar el proceso.`,
        {
        }
      );
    }
    ...
    console.error("Error en finalizarTrabajo:", e);
